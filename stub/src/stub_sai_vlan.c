...
//    vlans[0].port_list = malloc(sizeof(sai_object_id_t) * PORT_NUMBER);
//
//    for (ii = 0; ii < PORT_NUMBER; ++ii) {
//        stub_create_object(SAI_OBJECT_TYPE_PORT, ii, &port);
//        vlans[0].port_list[ii].port_id = port;
//        vlans[0].port_list[ii].tagging_mode = SAI_VLAN_TAGGING_MODE_UNTAGGED;
//    }
 ...
 */
sai_status_t stub_set_vlan_attribute(_In_ sai_object_id_t vlan_id,
 ...
 *    Failure status code on error
 */
sai_status_t stub_get_vlan_attribute(_In_ sai_object_id_t     vlan_id,
 ...
 *    Failure status code on error
 */
sai_status_t stub_create_vlan(_Out_ sai_object_id_t *vlan_id,
 ...
 *    Failure status code on error
 */
sai_status_t stub_add_ports_to_vlan(_Out_ sai_object_id_t      *vlan_member_id,
                                    _In_ sai_object_id_t        switch_id,
                                    _In_ uint32_t               attr_count,
                                    _In_ const sai_attribute_t *attr_list)
{
...
//    v->port_list = realloc(v->port_list, new_size * sizeof(sai_object_id_t));
//
//    if (v->port_list == NULL) {
//        STUB_LOG_ERR("Error: memory allocation for removing a vlan failed.\n");
//        return SAI_STATUS_NO_MEMORY;
...
//    memcpy(v->port_list + old_size, port_list, port_count * sizeof(sai_object_id_t));

    return SAI_STATUS_SUCCESS;
}

//static int remove_a_port_from_vlan(struct __vlan* v, sai_object_id_t port)
//{
//    int i, j;
//    int deleted = 0;
//
...
//
//            (v->number_of_ports)--;
//            v->port_list = realloc(v->port_list, sizeof(sai_object_id_t) * v->number_of_ports);
//
//            break;
//        }
//    }
 ...
 *    Failure status code on error
 */
sai_status_t stub_remove_ports_from_vlan(_In_ sai_object_id_t vlan_member_id)
{
    STUB_LOG_ENTER();
 ...
 *    Failure status code on error
 */
sai_status_t stub_get_vlan_stats(_In_ sai_object_id_t        vlan_id,
                               ...
                               void                          *arg)
{
    ...
    if (SAI_STATUS_SUCCESS != (status = stub_create_object(SAI_VLAN_ATTR_STP_INSTANCE, 1, &value->oid))) {
        return status;
    }
...
sai_status_t stub_vlan_stp_set(_In_ const sai_object_key_t *key, _In_ const sai_attribute_value_t *value, void *arg)
{
    ...
    if (SAI_STATUS_SUCCESS != (status = stub_object_to_type(value->oid, SAI_VLAN_ATTR_STP_INSTANCE, &stp_id))) {
        return status;
    }
